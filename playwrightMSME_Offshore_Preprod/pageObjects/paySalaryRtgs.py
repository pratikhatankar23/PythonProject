...
class PaySalaryRtgsPage(PaySalaryNeftPage):
    def pay_salary_rtgs(self, salary_data):
        try:
    ...
    def _save_debug_artifacts(self) -> Path:
        artifact_dir = Path("artifacts") / f"pay_salary_rtgs_{time.strftime('%Y%m%d_%H%M%S')}"
