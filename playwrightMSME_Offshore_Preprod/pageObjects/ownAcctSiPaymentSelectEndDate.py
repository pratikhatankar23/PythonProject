...
from pageObjects.ownAcctSiPayment import OwnAcctSiPaymentPage


class OwnAcctSiPaymentSelectEndDatePage(OwnAcctSiPaymentPage):
    def own_account_si_payment_select_end_date(self, transfer_data):
        try:
            self._handle_post_login_popups()
            self.navigate_to_own_account_transfer()
            self.enter_own_account_si_payment_select_end_date_details(transfer_data)
    ...
    def enter_own_account_si_payment_select_end_date_details(self, transfer_data):
    ...
    def _missing_own_account_si_review_values(self, transfer_data, review_text):
        missing_values = super()._missing_own_account_si_review_values(
            transfer_data,
            review_text,
        )

        if re.search(r"Standing\s+Instruction\s+Schedule", review_text, re.I):
            si_schedule_values = (transfer_data["si_instruction_schedule"],)
            if not any(
                self._normalize_text(value) in review_text
                for value in si_schedule_values
            ):
                missing_values.append(si_schedule_values)
    ...
    def _wait_for_end_date_field(self, timeout: float = 30):
        ...
        while time.monotonic() < deadline:
            ...
            if error_match and "select end date" not in body_text.lower():
                raise AssertionError(
                    "Own account SI End Date field failed to load: "
    ...
    def _save_debug_artifacts(self) -> Path:
        artifact_dir = (
            Path("artifacts")
            / f"own_acct_si_payment_select_end_date_{time.strftime('%Y%m%d_%H%M%S')}"
        )
