...
class InternalSiPaymentPage(NeftSiPaymentPage):
    def internal_si_payment(self, transfer_data):
        try:
            self._handle_post_login_popups()
            self.navigate_to_pay_beneficiary()
            self.enter_internal_si_payment_details(transfer_data)
            self.verify_internal_si_review_transaction(transfer_data)
    ...
    def enter_internal_si_payment_details(self, transfer_data):
    ...
    def verify_internal_si_review_transaction(self, transfer_data):
        ...
        while time.monotonic() < deadline:
            ...
            missing_values = self._missing_internal_si_review_values(
                transfer_data,
                review_text,
            )
        ...
        if missing_values:
            raise AssertionError(
                "Review Transaction page is missing expected internal SI value(s): "
                f"{missing_values}\n\nReview text:\n{review_text}"
            )

    def _missing_internal_si_review_values(self, transfer_data, review_text):
    ...
    def _save_debug_artifacts(self) -> Path:
        artifact_dir = Path("artifacts") / f"internal_si_payment_{time.strftime('%Y%m%d_%H%M%S')}"
