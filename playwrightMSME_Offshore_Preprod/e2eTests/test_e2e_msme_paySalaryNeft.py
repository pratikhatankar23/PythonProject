...
PAY_SALARY_NEFT_DATA_PATH = PROJECT_ROOT / "TestData" / "paySalaryNeft.json"
...
@pytest.mark.parametrize(
    "salary_data",
    _load_json_data(PAY_SALARY_NEFT_DATA_PATH),
    ids=lambda data: data.get(
        "case_id",
        data.get("employeeName", "pay_salary_neft"),
    ),
)
def test_pay_salary_neft(page, base_url, login_data, salary_data):
    ...
    salary_payment = PaySalaryNeftPage(page)
    salary_payment.pay_salary_neft(salary_data)
