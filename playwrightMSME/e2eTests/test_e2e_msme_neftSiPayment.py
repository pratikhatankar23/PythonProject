...
NEFT_SI_DATA_PATH = PROJECT_ROOT / "TestData" / "neftSiPayment.json"
...
@pytest.mark.parametrize(
    "neft_si_data",
    _load_json_data(NEFT_SI_DATA_PATH),
    ids=lambda data: data.get("case_id", data.get("payeename", "neft_si_payment")),
)
def test_neft_si_payment(page, base_url, login_data, neft_si_data):
    ...
    neft_si_payment = NeftSiPaymentPage(page)
    neft_si_payment.neft_si_payment(neft_si_data)
