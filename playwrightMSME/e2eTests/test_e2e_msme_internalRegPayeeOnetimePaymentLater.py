...
from pageObjects.internalRegPayeeOnetimePaymentLater import (
    InternalRegPayeeOnetimePaymentLaterPage,
)
...
INTERNAL_PAYMENT_DATA_PATH = (
    PROJECT_ROOT / "TestData" / "internalRegPayeeOnetimePaymentLater.json"
)
...
@pytest.mark.parametrize(
    ...
    ids=lambda data: data.get(
        "case_id",
        data.get("payeename", "internal_reg_payee_onetime_payment_later"),
    ),
)
def test_internal_registered_payee_onetime_payment_later(
    page,
    base_url,
    login_data,
    internal_payment_data,
    ...
    internal_payment = InternalRegPayeeOnetimePaymentLaterPage(page)
