...
from pageObjects.paySalaryImps import PaySalaryImpsPage


PROJECT_ROOT = Path(__file__).resolve().parents[1]
LOGIN_DATA_PATH = PROJECT_ROOT / "TestData" / "msmeLogin.json"
PAY_SALARY_IMPS_DATA_PATH = PROJECT_ROOT / "TestData" / "paySalaryImps.json"
...
@pytest.mark.parametrize(
    "salary_data",
    _load_json_data(PAY_SALARY_IMPS_DATA_PATH),
    ids=lambda data: data.get(
        "case_id",
        data.get("employeeName", "pay_salary_imps"),
    ),
)
def test_pay_salary_imps(page, base_url, login_data, salary_data):
    ...
    salary_payment = PaySalaryImpsPage(page)
    salary_payment.pay_salary_imps(salary_data)
